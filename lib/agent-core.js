...
// count = velas que cubren 30 días (5m usa 10 días y 1D usa 200 velas:
// con menos no hay histórico para los indicadores; con más, 5m pesa demasiado)
const TF = {
  '5m' : {binance:'5m',  cb:300,   secs:300,   count:2880, tp:0.004, sl:0.007},
...
/* Filtro de calidad de señales EMA: solo se opera a favor de la tendencia
   (precio respecto a EMA50), con bandas de RSI más estrictas y un
   enfriamiento mínimo entre señales del mismo tipo para no repicar. */
const SIG = {trendEma:50, cooldown:10, rsiBuy:[48,68], rsiSell:[32,52]};
...
function computeSignals(cs){
  ...
  const e50 = ema(closes, SIG.trendEma);
  const a = atr(cs,14);
  const zones = detectZones(cs, a);
  const sigs = [];
  let lastBuy = -Infinity, lastSell = -Infinity;
  for(let i=30;i<cs.length;i++){
    const up = e9[i-1]<=e21[i-1] && e9[i]>e21[i];
    const dn = e9[i-1]>=e21[i-1] && e9[i]<e21[i];
    if(up && r[i]!=null && r[i]>=SIG.rsiBuy[0] && r[i]<=SIG.rsiBuy[1]
       && closes[i]>e50[i] && i-lastBuy>SIG.cooldown){
      sigs.push({i, kind:'ema', type:'BUY',  price:closes[i], time:cs[i].time});
      lastBuy = i;
    }
    if(dn && r[i]!=null && r[i]>=SIG.rsiSell[0] && r[i]<=SIG.rsiSell[1]
       && closes[i]<e50[i] && i-lastSell>SIG.cooldown){
      sigs.push({i, kind:'ema', type:'SELL', price:closes[i], time:cs[i].time});
      lastSell = i;
    }
  }
  ...
  return {sigs, zones, e9, e21, e50, r, a};
}
...
function featurePrep(cs){
  ...
  const aS = sma(a.map(v=>v==null?0:v), 50); // media del ATR: régimen de volatilidad
  const volS = sma(cs.map(c=>c.volume||0), 20);
  return function(i, dir){ // dir: +1 largo (BUY), -1 corto (SELL)
    if(i<70 || i>=cs.length || a[i]==null || r[i]==null) return null;
    ...
    const ang = 2*Math.PI*d.getUTCHours()/24;
    const angD = 2*Math.PI*d.getUTCDay()/7;
    ...
    return [dir, r[i]/100, (e9[i]-e21[i])/at, (c.close-e21[i])/at, at/c.close*100,
            ...
            (c.close-e50[i])/at,
            (aS[i]&&aS[i]>0) ? Math.min(5, at/aS[i]) : 1,
            Math.sin(angD), Math.cos(angD)];
  };
}
...
return {TF, ZONE_MIN, ZONE_WIDTH, VOL_CONFIRM, SIG, FEATURES,
